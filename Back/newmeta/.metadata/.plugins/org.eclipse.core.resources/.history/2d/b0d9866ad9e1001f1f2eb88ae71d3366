...
public class CsvReaderService {
    ...
    private static final String FILE_PATH = "C:\\Users\\user\\Desktop\\test/test_product_event_log_v12.csv";
    ...
    public void processCsvLine() {
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_PATH))) { // ✅ CSV 파일 읽기
            ...
            // ✅ 파일의 끝에 도달하면 종료 메시지를 출력하고 로직 종료
            if (line == null) {
    ...
    private void processRow(String line) {
        ...
        try {
            ...
            String productName = data[1].trim(); // ✅ 제품명
            String hubType = data[2].trim(); // ✅ 허브 유형
            String eventType = data[3].trim(); // ✅ 이벤트 유형
            Date eventTime = parseDate(data[4].trim()); // ✅ 이벤트 발생 시간
