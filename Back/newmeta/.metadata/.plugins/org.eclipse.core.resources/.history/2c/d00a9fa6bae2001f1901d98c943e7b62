...
public class ProductEventLogDTO {
    ...
    private Long seq; // 이벤트 로그의 고유 식별자 (Primary Key 역할 가능)

    private Date eventTime; // 이벤트가 발생한 날짜 및 시간 정보를 저장하는 필드

    private String epcCode; // EPC(Electronic Product Code), 제품의 고유 식별 코드

    private String productName; // 제품의 이름을 저장하는 필드

    private String eventType; // 이벤트의 유형 (예: 입고, 출고, 이동 등)
    ...
    private boolean isAnomaly; // 이벤트가 정상적인지 여부를 나타내는 필드 (이상 탐지 여부)
}
