...
public class ReportedController {
	...
	/**
	 * /report : ProductEventLogDTO를 받아 로직 실행 1) Product/Event/Hub 조회 or 생성 2)
	 * ProductEventLog 저장 3) DB에서 해당 EPC의 모든 로그 조회 4) 1개 이상이면 이상치
	 * 탐지(detectAnomalies) 5) 국내산은 10개 이상, 수입산은 8개 이상이면 AI 호출 6) 최근 10개 로그 콘솔
	 * 출력(디버깅용)
	 */
	@PostMapping("/report")
	public void reported(@RequestBody ProductEventLogDTO dto) {
		...
		List<ProductEventLog> events = productEventLogRepo
				.findAllByProduct_EpcCodeOrderByProductEventLogIdAsc(dto.getEpcCode());
		...
		// [E] 최근 10개 로그 콘솔 출력
		List<ProductEventLog> last10 = productEventLogRepo
				.findTop10ByProduct_EpcCodeOrderByProductEventLogIdDesc(dto.getEpcCode());
	...
	/**
	 * 기존 AnomalyDetectionService 로직을 모두 포함: 1) 국내산 첫 이벤트 검사 2) 수입산 첫 이벤트 검사 3) 직접
	 * 판매 이벤트 4) 허가되지 않은 이벤트 5) 이벤트 순서 오류 6) AI 호출 (임계치: 국내10, 수입8) 7) 이상 없으면 전부 정상
	 * 처리
	 */
...
//							.filter(e -> e.getProduct().getEpcCode().equals(anomalyEpc)).findFirst();
//
//					if (match.isPresent()) {
//						if (isAnomaly) {
...
//		}
	...
	/**
	 * 이상 로그 저장 - 중복 검사 (epcCode + eventTime) - productEventLog.isAnomaly = true -
	 * anomaly_log 테이블 저장 - 웹소켓 알림
	 */
	private void saveAnomalyLog(ProductEventLog eventLog, String anomalyType, String reason) {
		...
		AnomalyLog anomalyLog = AnomalyLog.builder().anomalyType(anomalyType).reason(reason).productEventLog(eventLog)
				.build();
		anomalyLogRepository.save(anomalyLog);

		// 3) WebSocket 알림 전송
		AnomalyDTO alert = AnomalyDTO.builder().anomalyId(anomalyLog.getAnomalyId())
				.anomalyType(anomalyLog.getAnomalyType()).reason(anomalyLog.getReason())
				.anomalyType(eventLog.getEvent().getEventType()).epcCode(eventLog.getProduct().getEpcCode())
				.productName(eventLog.getProduct().getProductName()).hubType(eventLog.getHub().getHubType())
				.latitude(eventLog.getHub().getLatitude()).longitude(eventLog.getHub().getLongitude())
				.anomalyTimestamp(eventLog.getEventTime()).build();
