...
// [기존 ReportedController에 있던 임포트들 복사 + 필요한 것 유지]
...
public class ReportedService {
	...
	public void reported(ProductEventLogDTO dto) {
	...
	/**
	 * [기존 private 메서드들 전부 이동]
	 * - 접근제한자는 private 그대로 유지
	 * - 로직/코드 100% 동일
	 */
			...
			if (!aiResults.isEmpty()) {
				for (PredictionResultDTO result : aiResults) {
					...
					if (matchingEvents.size() >= 4) {  // ✅ 4번째 이벤트가 있는지 확인
						ProductEventLog fourthEvent = matchingEvents.get(4);  // ✅ 4번째 이벤트 선택 (인덱스 4번째는 실제론 index=3이지만, 주의!)
						if (isAnomaly) {
